...
describe('Test for /api/items', () => {
    ...
    test('It should update the title, description, shortDescription of the desired item', async () => {
        ...
        const response = await client.put(`/api/items/`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send(
            {
                id:uniqueItem._id,
        ...
        expect(response.body.shortDescription).toBe("updated item shortDescription")
    })
    ...
    test('It will given validation error while update the minPrice. When minPrice will be greater than maxprice of the desired item', async () => {
        ...
        expect(response.body.message).toBe("Max price should be greater than min price of the item.")
    })
    ...
    test('It should archive the desired item', async () => {
        const response = await client.get(`/api/items/set-archive/${uniqueItem._id}`)
        .set('Authorization', `Bearer ${accessToken}`)
    ...
    test('It should soft delete the desired item', async () => {
        const response = await client.delete(`/api/items/${uniqueItem._id}`)
        .set('Authorization', `Bearer ${accessToken}`)
...
// eslint-disable-next-line no-undef
afterAll(async () => {
    await client.get('/api/auth/signout')
                .set('Authorization', `Bearer ${accessToken}`)
    accessToken = null
    uniqueItem = null
