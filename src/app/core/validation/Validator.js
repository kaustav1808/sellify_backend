...
class Validator {
  ...
  validate(){
    if (isEmpty(this.rules)){
        throw new SLFYError(SLFY_NO_RULE_DEFINED,"No rule defined for running the validation.")
    }

    const {error} = this.schema.validate(this.params,this.options)

    if (error) {
        this.errors = error.details.map(err=>({ key: err.path[0], message: err.message }))
        return false
    }
